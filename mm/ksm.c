...
#ifdef CONFIG_KSM_DEBUG_TRACE
#define CREATE_TRACE_POINTS
#include <trace/events/ksm.h>
static int ksm_seq_id = 0;
#endif
...
extern int pagecmp(const void *, const void *, __kernel_size_t);
extern u32 calc_pagesum(const void *);
...
#if defined(CONFIG_ADAPTIVE_KSM)
...
static int memcmp_pages(struct page *page1, struct page *page2)
{
	...
	ret = pagecmp((long *)addr1, (long *)addr2, PAGE_SIZE);
	kunmap_atomic(addr2);
	kunmap_atomic(addr1);
	return ret;
}
...
#ifdef CONFIG_KSM_DEBUG_TRACE
                        trace_ksm_e(ksm_seq_id, ksm_pages_shared, ksm_pages_sharing, ksm_pages_unshared);
                        ksm_seq_id++;
#endif
